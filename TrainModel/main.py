...
def texture_dataset(split_Train_Test=0.9):
    xAll = np.load('data.npy')
    yAll = np.load('groundTruth.npy')
...
def main(numberOfEpochs = 500):
    ...
    if tf.keras.backend.image_data_format() == 'channel_first':
        normed_train_data = normed_train_data.reshape(
                                                                normed_train_data.shape[0], 
                                                                1, normed_train_data.shape[1], 
                                                                normed_train_data.shape[2])
        normed_test_data = normed_test_data.reshape(
                                                                normed_test_data.shape[0], 
                                                                1, normed_test_data.shape[1], 
                                                                normed_test_data.shape[2])
        input_shape = (
        ...
        normed_train_data = normed_train_data.reshape(
                                                                ...
                                                                normed_train_data.shape[2], 1)
        normed_test_data = normed_test_data.reshape(
                                                                normed_test_data.shape[0], 
                                                                normed_test_data.shape[1], 
                                                                normed_test_data.shape[2], 1)
        input_shape = (
                                        normed_test_data.shape[1], normed_test_data.shape[2], 1
                                    )

    model = tf.keras.Sequential([
                    layers.Reshape(
                                            target_shape=[normed_train_data.shape[1], normed_train_data.shape[2], 1], 
                                            input_shape=input_shape),
                    layers.Conv2D(30, (5, 5), 
                                            strides=(3, 3),
                                            padding='same', 
                                            activation=tf.nn.elu),
                    layers.MaxPooling2D((2, 2), (2, 2), 
                                            padding='same'),
                    layers.Conv2D(60, (3, 3), 
                                            strides=(2, 2),
                                            padding='same', 
                                            activation=tf.nn.elu),
                    layers.Conv2D(60, (3, 3), 
